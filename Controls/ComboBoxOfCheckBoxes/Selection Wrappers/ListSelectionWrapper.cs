...
namespace Controls.ComboBoxOfCheckBoxes
{
    ...
    /// Useful in the ComboBoxOfCheckBoxes. It holds a reference to the List[Index] Item and 
    ...
    public class ListSelectionWrapper<T> : List<ObjectSelectionWrapper<T>>
    {
        ...
        private ObjectSelectionWrapper<T> CreateSelectionWrapper(IEnumerator Object)
        {
            ...
            if (CI == null)
                throw new Exception($"The selection wrapper class {typeof(ObjectSelectionWrapper<T>)} must have a constructor with ({typeof(T)} Item, {this.GetType()} Container) parameters.");
