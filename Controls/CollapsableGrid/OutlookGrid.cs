...
//NOTE: While DataGridView will remain data bound the ROWS WILL NOT
...
namespace Controls
{
    ...
    public partial class OutlookGrid : DataGridView
    {
        ...
        public void BindData(object dataSource, string dataMember, bool clear = true)
        {
            this.DataMember = DataMember;
            if (dataSource == null)
            {
                this.dataSource = null;
                if(clear) Columns.Clear();
            }
            else
            {
                this.dataSource = new DataSourceManager(dataSource, dataMember);
                if (clear) SetupColumns();
                FillGrid(null);
            }
        }
            ...
            if (groupingStyle == null)
            {
                foreach (DataSourceRow r in list)
                {
                    ...
                    foreach (object val in r)
                    {
                        ...
                        bool isBool;
                        if (bool.TryParse(val.ToString(), out isBool))
                        {
