...
namespace Controls
{
    ...
    public class OutlookgGridDefaultGroup : IOutlookGridGroup
    {
        ...
        public virtual string Text
        {
            get {
                if (column == null)
                    return $"Unbound group: {Value.ToString()} ({(itemCount == 1 ? "1 item" : itemCount.ToString() + " items")})";
                else
                    return $"{column.HeaderText}: {Value.ToString()} ({(itemCount == 1 ? "1 item" : itemCount.ToString() + " items")})"; 
                }
            set { text = value; }
        }
    ...
    public class OutlookGridAlphabeticGroup : OutlookgGridDefaultGroup
    {
        ...
        public override string Text
        {
            get
            {
                return $"Alphabetic: {Value} ({(itemCount == 1 ? "1 item" : itemCount + " items")})";
            }
            set { text = value; }
        }
